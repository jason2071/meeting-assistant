...
- **Layout & sessions**: the body is an app shell — `aside.sidebar` (session list, ＋New, font slider, 🏠) + `main.view` holding three sections toggled by `showView(name)`: `#viewHome` (landing + session list), `#viewCurrent` (the live UI — settings/tabs/voice/screen/input/`#results`), `#viewOld` (readonly past session). Sessions persist in `localStorage`: index `ma_sessions` + per-session `ma_sess_<id>` ({id,title,createdAt,updatedAt,provider,model,items}); each `item` is `{q,mode,hadImage,raw}` where `raw` is the answer markdown (qa) or `JSON.stringify(est)` — stored raw, re-rendered via `renderSessionInto`/`mdToHtml`/`renderEstimate` (not saved HTML). `submit()` calls `saveItem()` on success; `newSession()`/`deleteSession()`/`openSession()` manage the rest. Font size = `--fs` CSS var driven by a slider, persisted as `ma_fontsize`.
