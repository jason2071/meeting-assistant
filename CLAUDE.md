...
- **Layout & sessions (lobby → chat)**: no sidebar/navbar. `main.view` holds three sections toggled by `showView(name)`: `#viewHome` (lobby — the `#settings` panel + `▶ เริ่มแชท` `startBtn` + `#homeList` session list), `#viewCurrent` (`.chat-wrap`: sticky header with ← home/title/clear, `#results` chat-msgs, `.composer` with mode tabs/voice/screen/input), `#viewOld` (readonly chat). Conversation renders as **chat bubbles** — `addUserMsg()` (right, teal) + `addAiMsg()` (left, holds `.ans`); `scrollBottom()` after each. Sessions persist in `localStorage`: index `ma_sessions` + per-session `ma_sess_<id>` ({id,title,...,items}); each `item` is `{q,mode,hadImage,raw}` (raw = markdown / `JSON.stringify(est)`), rebuilt by `renderSessionInto` (chronological). `submit()`→`saveItem()`; `newSession`/`deleteSession`(+confirm)/`openSession`/`initSessions` (always lands on `home`/settings by default, restoring the last session into memory so clicking it reopens the chat). Settings live only on home. Font size = `--fs` slider, persisted `ma_fontsize`.
