...
// (classic script; loaded last — top-level globals shared. liveOn ประกาศใน 04-voice.js)
//
// flow: WS connect → setup(inputAudioTranscription) → mic→PCM16/16k→realtimeInput.audio (base64)
//       → server ส่ง serverContent.inputTranscription.text (สด) + turnComplete (จบประโยค)
//       → autoSend ส่งเข้า submit; ไม่งั้นโชว์ preview + ปุ่มส่งสว่าง
...
// Float32 (native rate) → downsample เป็น 16kHz → PCM16 LE → base64 (Gemini Live ต้องการ pcm 16k mono)
function _f32To16kPCM16B64(f32, inRate){
  ...
  for(let i=0;i<outLen;i++){
    let s=f32[Math.floor(i*ratio)]; s=Math.max(-1,Math.min(1,s||0));
    view.setInt16(i*2, s<0?s*0x8000:s*0x7FFF, true);
  }
  let bin="", u=new Uint8Array(buf);
  for(let i=0;i<u.length;i++) bin+=String.fromCharCode(u[i]);
  return btoa(bin);
}
function _liveFinalize(){   // เอา turn ที่ถอดได้ไปใช้ (autoSend ส่งเลย / ไม่งั้นรอกดส่ง)
...
async function startLive(){
  ...
  liveTr = ($("liveTranslate") && $("liveTranslate").value) || "";   // "" = ไม่แปล
  ...
  liveWS.onopen=()=>{
    // model live เป็น native-audio → ต้อง responseModalities AUDIO (TEXT ถูก reject); เราทิ้ง audio output อ่านแค่ transcription
    const setup = liveTr
      ? { model:"models/"+TRANSLATE_MODEL,            // โหมดแปล: voice-to-voice → อ่าน outputTranscription (ข้อความที่แปลแล้ว)
          generationConfig:{ responseModalities:["AUDIO"] },
          inputAudioTranscription:{}, outputAudioTranscription:{},
          translationConfig:{ targetLanguageCode:liveTr, echoTargetLanguage:true } }
      : { model:"models/"+LIVE_MODEL,                  // โหมดเรียลไทม์ปกติ: อ่าน inputTranscription (ต้นฉบับ)
          generationConfig:{ responseModalities:["AUDIO"] },
  ...
  liveWS.onmessage=async(ev)=>{
    ...
    if(sc){
      // แปล → ใช้ outputTranscription (แปลแล้ว); ปกติ → inputTranscription (ต้นฉบับ). ignore audio (modelTurn) ไม่เล่น
...
function _liveBeginCapture(){
  try{
    liveCtx=new (window.AudioContext||window.webkitAudioContext)();   // native rate (เช่น 48k) แล้ว downsample เอง
    ...
    liveSrc.connect(liveProc); liveProc.connect(liveCtx.destination);  // processor ไม่เขียน output → เงียบ ไม่ echo
