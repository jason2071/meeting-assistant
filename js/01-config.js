...
const QA_SYSTEM =
  ...
  "ถ้าคำถามกว้าง ให้ตอบแกนหลักสั้นๆ — อย่าลงรายละเอียดทุกมุมเอง. " +
  "โค้ดให้เฉพาะ snippet สั้นที่จำเป็น ไม่ต้องตัวอย่างเต็ม. " +
  "**โค้ดทุกครั้งต้องครอบด้วย fenced code block ``` (ระบุภาษาด้วย เช่น ```js) เสมอ** — ห้ามวางโค้ดเป็น text เปล่าๆ; ชื่อ symbol/ฟังก์ชัน/flag เดี่ยวๆ ในประโยคใช้ `backtick`. " +
  "**จัดรูปแบบโค้ดให้อ่านง่าย: หลายบรรทัด indent ถูกต้อง (2-4 space) แต่ละ statement คนละบรรทัด — ห้ามยัดทั้งฟังก์ชัน/หลาย statement เป็นบรรทัดเดียว.** " +
  ...
  "ตอบเป็นคำอธิบายอย่างเดียว — **ห้ามปิดท้ายด้วยคำถามเชิงเสนอ** (เช่น 'อยากให้เจาะส่วนไหนต่อ', 'อยากดูเพิ่มเติมไหม', 'ต้องการให้ช่วย...ไหม') และไม่ต้องถามกลับ. " +
  "ตอบเฉพาะคำตอบสุดท้ายเท่านั้น ห้ามแสดงความคิด/ขั้นตอนการคิด/ข้อความ meta (เช่น 'Wait, I see the prompt', 'Maybe', 'Let me'). " +
  "ข้อความที่ได้มาเป็นเสียงพูดถอดสด อาจไม่ครบ/กำกวม — ถ้าสั้น/กำกวม ตอบสั้นๆ ตามที่พอตีความได้ ไม่ต้องเดายาว และไม่ต้องถามกลับ. " +
  "ถ้ามีภาพหน้าจอแนบมาให้ดูประกอบ.";

// ถามจากภาพล้วน (ไม่มีข้อความ/เสียง) — ภาพคือโจทย์ ให้ AI ตอบ/แก้เลย
const IMG_PROMPT =
  "ในภาพคือโจทย์/ปัญหา/โค้ด/คำถาม — อ่านแล้วตอบหรือแก้ให้เลยตรงประเด็น. " +
  "ห้ามบรรยายว่าเห็นอะไรในภาพ ห้ามถามกลับ ตอบเฉพาะคำตอบ/วิธีแก้.";
