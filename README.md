...
- **🧵 จำบทสนทนา (multi-turn)** — ส่งประวัติ Q&A ใน 10 turn ล่าสุดให้ AI → ถามต่อเนื่องได้ ("อธิบายเพิ่ม"). toggle ในตั้งค่า (default เปิด; ปิด = ถามเดี่ยวประหยัด token)
- **⚡ Quick-ask + 💡 follow-up chips** — chip preset (อธิบายโค้ดนี้/หา bug/เขียน test…) กดส่งทันที; หลัง AI ตอบ มี chip คำถามต่อที่ AI แนะนำ กดถามต่อด้วยบริบทเดิม (toggle ปิดได้)
- **🎙 ถอดเสียง: สด / สด+AI** — toggle ในตั้งค่า:
  - **สด** = Web Speech (live ทีละคำ ฟรี) — ปุ่มเดียว toggle `🎤 เริ่มฟัง ⇄ ⏹ หยุด` + `✂️ ส่งเลย` (เหมือน สด+AI); หยุด=หยุดฟังเฉยๆ เริ่มใหม่ได้
...
`01-config` (providers/prompts) · `02-state` (storage/state/DOM/$/quickasks) · `03-models` (fetch model/device check) · `04-voice` (mode/voice/screen/toggles) · `05-llm` (buildRequest+history/streamLLM) · `06-render` (markdown/bubbles/chips) · `07-main` (submit/sessions/follow-up/init) · `08-overlay` (หน้าต่างลอย: Electron window + browser PiP, mirror+controls) · `09-hotkeys` (คีย์ลัด global/focus). `config.local.js` (optional) โหลดก่อน `js/*`
